...
def srd_to_file_structure(state: FileStructureState) -> FileStructureState:
    ...
    """Generates a file structure from the srd document."""
    ...
    try:
        ...
        if json_match:
            ...
            try:
                ...
                print("Invalid JSON Response:", json_string)
                raise e
...
def write_code_into_files(state: dict) -> dict:
 
    """Writes code into the generated files based on descriptions and
    adds requirements to requirements.txt."""
    ...
    for file_path in file_structure:
        ...
        prompt = f"""
        You are a FastAPI developer. Generate a complete Python file based **only** on the following description:  
        {description}  
...
def improve_code(state: FileStructureState) -> FileStructureState:
    ...
    """Applies improvements based on feedback, with human
...
def generate_tests(state: FileStructureState) -> FileStructureState:
    ...
    """
    Reads all the Python files in the generated project folder and generates a test cases
     based on its content. Test cases are stored in a "tests" subfolder as separate files.
    """
...
def reflect_on_errors(state: FileStructureState) -> FileStructureState:
    ...
    gives suggestions for improvements, and updates the state with feedback."""
...
def final_execution(state: FileStructureState) -> FileStructureState:
    ...
    """Runs the final version of the code."""
...
for fn in [srd_to_file_structure, create_files_tool, write_code_into_files, reflect_on_code,
           improve_code, generate_tests, run_code, reflect_on_errors, final_execution]:
    graph.add_node(fn.__name__, fn)

edges = [
    (START, 'srd_to_file_structure'),
    ('srd_to_file_structure', 'create_files_tool'),
    ('create_files_tool', 'write_code_into_files'),
    ('write_code_into_files', 'reflect_on_code'),
    ('reflect_on_code', 'improve_code'),
    ('improve_code', 'generate_tests'),
    ('generate_tests', 'run_code'),
    ('run_code', 'reflect_on_errors'),
    ('reflect_on_errors', 'improve_code'),
